 ...
 function hammerPinch(){
     //创建一个新的hammer对象并且在初始化时指定要处理的dom元素
     var hammertime = new Hammer(document.getElementById("subway"));
     hammertime.add(new Hammer.Pinch());
     //放大
     hammertime.on("pinchin", function (evt) {
         zoom('zoomOut', pos, evt);
     });
     //缩小
     hammertime.on("pinchout", function (evt) {
         zoom('zoomIn', pos, evt);
     });
 ...
 function zoom(zoomType, pos, evt) {
     ...
     } else if (zoomType == 'zoomOut') //缩小
     {
         if (scale == "0.5") {
             ...
             drwSw.reDrwname(cityCode, drwData, 24, 26);
             if (tip.isHighlight == true) { //高亮下的缩放
                 var lineId = $("#line-select-box").attr("line_id");
                 $("#g-line-select text").remove();
                 drwSw.reDrwZoomName(lineId, scale, 24, 26);
             }
             SW.dragMoveDown(dragObj);
         }
